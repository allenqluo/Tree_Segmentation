...
#         elif use_tower_height is False:
    ...
    def calc_waveform(self, pulse_width=0.2):
        # Normalize point density
        normalized_pt_density = self.ptDensity / np.sum(self.ptDensity)  # Normalize the point density

        # Convolve the normalized point density with a Gaussian to simulate LiDAR pulse
        waveform = gaussian_filter(normalized_pt_density, sigma=pulse_width / self.dz)

        # Save the waveform to the class
        self.waveform = waveform
    
    def calculate_cumulative_gap(self):
        """Calculates and saves the cumulative gap fraction to self.cumulative_gap."""
        gap_values = self.gap  # Assuming self.gap holds the gap fractions
        self.cumulative_gap = np.zeros_like(gap_values)

        # Initialize the first value
        self.cumulative_gap[0] = gap_values[0]

        # Calculate cumulative gap by multiplying the gaps
        for i in range(1, len(gap_values)):
            self.cumulative_gap[i] = self.cumulative_gap[i - 1] * gap_values[i]